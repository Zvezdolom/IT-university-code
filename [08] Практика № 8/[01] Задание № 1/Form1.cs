...
namespace _01__Задание___1
{
    public partial class Form1 : Form
    {
        ...
        public void CalcFunction()
        {
            ...
            {
                X[i] = XMin + Step * i;
                Y[i] = ((Math.Sqrt(1 + (Math.Pow(Math.E, Math.Sqrt(X[i])) + Math.Cos(Math.Pow(X[i], 2))))) / (Math.Abs(1 - Math.Pow(Math.Sin(X[i]), 3)))) + Math.Log(Math.Abs(2 * X[i]));
            }
        }
